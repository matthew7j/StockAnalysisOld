...
public class DatabaseEngine
{
    ...
    private void addYearData(int year, AnalysisEngine e) {
        ...
        try {
            ...
            for (int i = 0; i < e.currentAssets.size(); i++) {
                if (e.currentPositionYears.get(i).equals(year)) {
                    currentAssetYear = Integer.parseInt(String.valueOf(e.currentPositionYears.get(i)));
                    currentAssetIndex = i;
                }
            }

            if (year != e.year) {
                sql =   "INSERT INTO Stocks.StockQuarter (" +
                        "YearID, QuarterID, StockID, Price, P/E, Dividend Yield, Timeliness, Safety, HighProj, HighProjGain, " +
                        "HighProjAnnualReturn, LowProj, InsiderBuys, InsiderSells, Revenues Per Share, Cash Flow Per Share, Earnings Per Share, " +
